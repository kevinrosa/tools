function grd = dc_roms_get_grid(grd_file,scoord,tindex,~)
  ...
  for v = varlist
    ...
    try
      ...
      if strcmp(vname,'h')
        ...
        grd.(vname) = ncread(grd_file,'bath',[1 1 1],[1 Inf Inf])';
      end
    end
  end
...
%            ' scoord parameters are input as a vector'])
  ...
  z_r = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, ...
    rgrid, h', zeta', 0);
  grd.z_r = single(permute(z_r,[3 2 1]));
  clear z_r
  ...
  z_w = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, ...
    wgrid, h', zeta', 0);
  grd.z_w = single(permute(z_w,[3 2 1]));
  ...
  if nargin > 3
    ...
    z_u = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, ...
      ugrid, h', zeta', 0);
    grd.z_u = single(permute(z_u,[3 2 1]));
    ...
    z_v = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, ...
      vgrid, h', zeta', 0);
    grd.z_v = single(permute(z_v,[3 2 1]));
